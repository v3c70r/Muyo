...
void DescriptorManager::createDescriptorSetLayouts()
{
    // GBuffer layouts
    {
        ...
        setDebugUtilsObjectName(reinterpret_cast<uint64_t>(layout),
                                ...
                                "Gbuffer");
        m_mDescriptorSetLayouts["GBuffer"] = layout;
    }
    ...
    // GBuffer lighting layout
    {
        ...
        setDebugUtilsObjectName(reinterpret_cast<uint64_t>(layout),
                                VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT,
                                "Lighting");
        m_mDescriptorSetLayouts["Lighting"] = layout;
    }
}

void DescriptorManager::destroyDescriptorSetLayouts()
{
    for (auto& descriptorPair : m_mDescriptorSetLayouts)
    {
        vkDestroyDescriptorSetLayout(GetRenderDevice()->GetDevice(),
                                     descriptorPair.second, nullptr);
    }
    m_mDescriptorSetLayouts.clear();
}
    ...
    const UniformBuffer<PerViewData>& perViewData, VkImageView textureView)
{
    ...
    allocInfo.pSetLayouts = &m_mDescriptorSetLayouts["GBuffer"];
    ...
    // Prepare buffer descriptor
    {
        ...
        imageInfo.sampler = GetSamplerManager().getSampler(FRAME_SAMPLER);
